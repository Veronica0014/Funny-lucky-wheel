...
buttonOn.addEventListener("click", showConfetti);
...
const stop = () => {
  setTimeout(function () {
    confetti.stop();
  }, 13000); // 5000 is time that after 5 second stop the confetti ( 5000 = 5 sec)
};
...
button.addEventListener("mouseover", function () {
  button.style.left = `${Math.ceil(Math.random() * 120)}%`;
  button.style.top = `${Math.ceil(Math.random() * 120)}%`;
});
